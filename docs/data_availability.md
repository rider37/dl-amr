...
- **Minimal reference dataset** for the uncertainty-validation figures
  (Figs. 9, 10, E.1), bundled as `reference_data_minimal.tar.gz` (~410 MB
...
- **Cached wake-field data** for Figs. 4, 5, 8, D.1 and F.1, bundled as
...
- **Full OpenFOAM raw outputs** (time directories, force and probe
  histories) behind Figs. 3, 6, 7 and the tables. These are not bundled in
