...
static void reportPhiDivergence
(
    const Foam::fvMesh& mesh,
    const Foam::surfaceScalarField& phi,
    const Foam::labelList& faceMap,
    const Foam::vectorField& oldSf,
    const Foam::vectorField& oldCf,
    const Foam::word& stage
)
{
    using namespace Foam;
    scalarField sumPhi(mesh.nCells(), Zero);
    const labelUList& own = mesh.owner();
    const labelUList& nei = mesh.neighbour();
    forAll(own, facei)
    {
        sumPhi[own[facei]] += phi[facei];
        sumPhi[nei[facei]] -= phi[facei];
    }
    ...
    const label worst = findMax(mag(sumPhi)());
    Pout<< "DIVCHK " << stage << " max|sumPhi| " << mag(sumPhi[worst])
        << " cell " << worst << " C " << mesh.C()[worst] << nl;
    if (mag(sumPhi[worst]) > 1e-6)
    {
        for (const label facei : mesh.cells()[worst])
        {
            if (mesh.isInternalFace(facei))
            {
                const label oldF =
                    (facei < faceMap.size() ? faceMap[facei] : -2);
                Pout<< "  f " << facei << " old " << oldF
                    << " phi " << phi[facei]
                    << " Sf " << mesh.Sf()[facei]
                    << " Cf " << mesh.Cf()[facei];
                if (oldF >= 0 && oldF < oldSf.size())
                {
                    Pout<< " oldSf " << oldSf[oldF]
                        << " oldCf " << oldCf[oldF];
                }
                Pout<< nl;
            }
...
void Foam::dynamicRefine2DFvMesh::mapFields(const mapPolyMesh& mpm)
{
    ...
    // does not use old-time volumes ...
    {
        ...
        // Each split cell becomes original + 3 additional
        ...
        {
            const label oldCelli = cellMap[celli];
            if (oldCelli >= 0 && nSubCells[oldCelli] == 4)
            {
        ...
        for (surfaceScalarField& phi : fluxes)
        {
            ...
            for (const label facei : masterFaces)
            {
                ...
                else
                {
                    ...
                    // 2-D: span faces on empty patches are split, so they
                    // appear as master faces, but empty patches carry no
                    // finite-volume faces (zero-sized patch fields) and no
                    // flux; skip them instead of writing past the end.
                    if (i >= patchPhi.size())
                    {
                        continue;
                    }
    ...
    // or point). An example is the internal faces from hexRef4.
    {
        const labelList& faceMap = mpm.faceMap();
...
)
{
    ...
    // Pre-change ownership, to detect silent owner/neighbour swaps that
    // carry no flipFaceFlux entry (see the matching fix in unrefine()).
    const labelList oldOwnerRef(faceOwner());
    const labelList oldNeighbourRef(faceNeighbour());
    const vectorField oldSfRef(Sf().primitiveField());
    const vectorField oldCfRef(Cf().primitiveField());
    ...
    // Refresh fluxes on faces whose ownership changed silently across the
    // refinement (no flipFaceFlux entry is recorded for these): recompute
    // from the interpolated velocity, as mapFields() does for split faces.
    {
        ...
        for (surfaceScalarField& phi : fluxes)
        {
            ...
            {
                const label oldFacei = faceMap[facei];
                // Refresh ONLY faces whose orientation actually reversed
                // across the topology change (geometric test: the new area
                // vector opposes the mapped-from face's). Anything else
                // carries a valid mapped flux; overwriting it with
                // interpolate(U)&Sf injects mass errors at every topology
                // event and shows up as drag oscillations.
                if
                (
                    oldFacei >= 0
                 && oldFacei < oldSfRef.size()
                 && (
                        ((Sf()[facei] & oldSfRef[oldFacei]) < 0)
                     || (
                            mag(magSf()[facei] - mag(oldSfRef[oldFacei]))
                          > 0.01*mag(oldSfRef[oldFacei])
                        )
                    )
                )
                {
            ...
            if (debug)
            {
                reportPhiDivergence
                (
                    *this, phi, faceMap, oldSfRef, oldCfRef, "refine"
                );
            }
        }
    }
...
)
{
    ...
    // (TBD: should be in changeMesh if no inflation?)
    // Pre-change ownership, to detect silent owner/neighbour swaps that
    // carry no flipFaceFlux entry (renumbering across removed cells).
    const labelList oldOwnerUnref(faceOwner());
    const labelList oldNeighbourUnref(faceNeighbour());
    const vectorField oldSfUnref(Sf().primitiveField());
    const vectorField oldCfUnref(Cf().primitiveField());
    scalarField oldPhiUnref;
    if (foundObject<surfaceScalarField>("phi"))
    {
        oldPhiUnref = lookupObject<surfaceScalarField>("phi").primitiveField();
    }
    ...
    if (debug)
    {
        Pout<< "unrefine: flipFaceFlux count "
            << map().flipFaceFlux().size()
            << " faces " << map().flipFaceFlux().sortedToc() << endl;
    }
    ...
    */
    ...
    // Correct the flux for modified faces.
    {
        ...
        for (surfaceScalarField& phi : fluxes)
        {
                ...
                if (reversePointMap[oldPointi] < 0)
                {
                    ...
                    if (facei >= 0)
                    {
                        ...
                        else
                        {
                            ...
                            // 2-D: merged span faces live on empty patches
                            // (zero-sized patch fields, no flux) - skip.
                            if (i >= patchPhi.size())
                            {
                                continue;
                            }
            ...
            // Refresh ONLY genuinely flipped faces (new owner maps onto the
            // old neighbour); see the matching comment in refine(). Faces
            // merely re-pointed to the merged master keep their orientation
            // and their mapped flux is valid.
            {
                const labelList& faceMap = map().faceMap();
                const labelList& cellMap = map().cellMap();
                for (label facei = 0; facei < nInternalFaces(); ++facei)
                {
                    ...
                    (
                        oldFacei >= 0
                     && oldFacei < oldSfUnref.size()
                     && (
                            ((Sf()[facei] & oldSfUnref[oldFacei]) < 0)
                         || (
                                mag(magSf()[facei] - mag(oldSfUnref[oldFacei]))
                              > 0.01*mag(oldSfUnref[oldFacei])
                            )
                        )
                    )
                    {
            ...
            if (debug)
            {
                reportPhiDivergence
                (
                    *this, phi, map().faceMap(), oldSfUnref, oldCfUnref,
                    "unrefine"
                );
            }
            ...
            if (debug)
            {
                ...
                for (label facei = 0; facei < nInternalFaces(); ++facei)
                {
                    if (mag(phi[facei] - phiU[facei]) > 1e-12)
                    {
                        const label own = faceOwner()[facei];
                        const label nei = faceNeighbour()[facei];
                        Pout<< "STALE face " << facei
                            << " oldFace " << faceMap[facei]
                            << " oldSf " << (faceMap[facei] >= 0
                                && faceMap[facei] < oldSfUnref.size()
                              ? oldSfUnref[faceMap[facei]]
                              : vector::zero)
                            << " newSf " << Sf()[facei]
                            << " oldPhi " << (faceMap[facei] >= 0
                                && faceMap[facei] < oldPhiUnref.size()
                              ? oldPhiUnref[faceMap[facei]]
                              : GREAT)
                            << " phi " << phi[facei]
                            << " phiU " << phiU[facei]
                            << " own " << own
...
Foam::labelList Foam::dynamicRefine2DFvMesh::selectUnrefinePoints
(
...
void Foam::dynamicRefine2DFvMesh::checkEightAnchorPoints
(
...
bool Foam::dynamicRefine2DFvMesh::updateTopology()
{
    ...
    if (time().timeIndex() > 0 && time().timeIndex() % refineInterval == 0)
    {
        ...
        {
            label nf=0, nf0=0;
            const labelList& cl = meshCutter_.cellLevel();
            forAll(vFld, ci)
            {
                if (vFld[ci] > 0.5)
                {
            ...
            Pout<< "FLAGSTAT nCells=" << nCells()
                << " flag=" << nf << " flagL0=" << nf0
                << " marked=" << refineCell.count() << nl;
            // flagL0 셀들의 x-분포 (스필 위치 특정)
            {
                label bins[6] = {0,0,0,0,0,0};
                label shown = 0;
                forAll(vFld, ci)
                {
                    if (vFld[ci] > 0.5 && cl[ci] == 0)
                    {
                ...
                Pout<< "  L0X x<2:" << bins[0] << " 2-5:" << bins[1]
                    << " 5-10:" << bins[2] << " 10-20:" << bins[3]
                    << " 20-39:" << bins[4] << " >39:" << bins[5] << nl;
            }
        }
...

        {
            ...
            if (debug && pointsToUnrefine.size())
            {
                Pout<< "UNSEL n=" << pointsToUnrefine.size() << nl;
                const pointField& pts = points();
                for
                (
                    label i = 0;
                    i < min(label(15), pointsToUnrefine.size());
                    ++i
                )
                {
                    const label pointi = pointsToUnrefine[i];
                    Pout<< "  P " << pts[pointi];
                    for (const label celli : pointCells()[pointi])
                    {
                        Pout<< " f" << vFld[celli];
