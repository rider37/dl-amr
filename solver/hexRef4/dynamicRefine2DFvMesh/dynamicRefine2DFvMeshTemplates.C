...
)
{
    ...
    {
        // Fields listed in correctFluxes are set consistently from the
        // interpolated velocity in mapFields(); hull-averaging afterwards
        // (upstream marks that path "Untested") re-breaks discrete
        // divergence on the new faces, so leave those fields alone.
        if (correctFluxes_.found(sFld.name()))
        {
            continue;
        }
